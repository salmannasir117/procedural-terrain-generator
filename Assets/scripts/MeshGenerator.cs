...
public class MeshGenerator : MonoBehaviour
{
    ...
    private Mesh create_plane(float grid_size, int grid_verts_per_side, float x_off, float y_off) {
        //modify to add a row and column 
        //currently, 
        Vector3[] verts = new Vector3[(grid_verts_per_side + 1) * (grid_verts_per_side + 1)];  	// the vertices of the mesh
	    int[] tris = new int[(2 * (grid_verts_per_side ) * (grid_verts_per_side)) * 3];      	// the triangles of the mesh (triplets of integer references to vertices)
	    Mesh mesh = new Mesh();
        Color[] colors = null;
        ...
        int ntris = 0;
        //set bound on i so that it stops before the second row
        for (int i = 0; i < verts.Length - (grid_verts_per_side + 1) - 1; i++) {
            if (i % (grid_verts_per_side) != 0 || i == 0) {
                int tl, tr, bl, br;
                tl = i;
                tr = i + 1;
